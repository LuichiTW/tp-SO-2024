...
void fSUM(enum lista_registros_CPU regLetraA, enum lista_registros_CPU regLetraB)
{
    ...
    case 4:
        ...
        {
        case 1:
            *(uint32_t *)regA += *(uint8_t *)regB;
            break;
        case 4:
            *(uint32_t *)regA += *(uint32_t *)regB;
            break;
            break;
        }
        break;
...
void fIO_STDIN_READ(char interface[], enum lista_registros_CPU Direccion, enum lista_registros_CPU Tamanio)
{ //! OBLIGATORIO
    int dir_logica = *((int *)obtenerRegistro(Direccion));
...
void fIO_STDOUT_WRITE(char interface[], enum lista_registros_CPU Direccion, enum lista_registros_CPU Tamanio)
{ //! OBLIGATORIO
    int dir_logica = *((int *)obtenerRegistro(Direccion));
    int registro_tamanios = *((int *)obtenerRegistro(Tamanio));
    t_lista_dir_fisicas lista_dir = obtener_direcciones_fisicas(dir_logica, registro_tamanios);
